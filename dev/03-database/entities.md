...
7. `job_embeddings`: Embeddings for semantic matching generated from the job description (`embedding_id`).
8. `skills`: Master skill table (`skill_id`).
9. `jobs`: Job postings (`job_id`).
10. `candidate_skills`: Bridge table for Candidate M-N Skill (Composite PK: `candidate_id`, `skill_id`).
11. `job_skills`: Bridge table for Job M-N Skill (Composite PK: `job_id`, `skill_id`).
12. `applications`: Bridge table for Candidate M-N Job (`application_id`).
13. `matches`: Stores the result of candidate-job matching (`match_id`).
14. `application_status_history`: Tracks changes in application statuses (`history_id`).\n